...
local function logMoreRequestDetails(debugInfos)	
	debugInfos["ServerUtcTime"] = os.date("!%c")
...
local function doCookieLog(debugInfos)
    for key, value in pairs(debugInfoArray) do
		if (debugInfos[key] == nil) then
			debugInfos[key] = value
		end
	end

    local cookieNameValues = { }
	for key, value in pairs(debugInfos) do
		cookieNameValues[key] = value
	end

    iHelpers.response.setCookie(QUEUEIT_DEBUG_KEY, table.concat(cookieNameValues, "|"), 0, nil)    
    debugInfoArray = debugInfos
end
